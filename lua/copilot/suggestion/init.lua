...
local function accept_in_insert_mode(new_text, range)
  ...
  local adjusted_first_line, outdent = suggestion_util.get_display_adjustments(
    first_line,
  ...
  lines[1] = adjusted_first_line
  new_text = table.concat(lines, "\n")

  local right_delete_count = 0
  if range["end"].line == line and range["end"].character > col0 then
    right_delete_count = range["end"].character - col0
  end
  ...
  local keys =
    string.rep("<Left><Del>", outdent)
    .. string.rep("<Del>", right_delete_count)
    .. recall
    .. "<End>"
